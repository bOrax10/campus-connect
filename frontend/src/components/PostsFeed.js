    ...
    const feed = [
        {
          ...
          "username": "aravind.s",
          'time': '1hr ago',
          'description': "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque cursus felis vel convallis hendrerit. Aliquam erat volutpat. Mauris non est. ",
          'tags': ["Software", "AI"],
          'comments': 3,
        ...
        },
        {
          ...
          "username": "aravind.s",
          'time': '1hr ago',
          'description': "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque cursus felis vel convallis hendrerit. Aliquam erat volutpat. Mauris non est.https://medium.com/@techsuneel99/message-queue-in-node-js-with-bullmq-and-redis-7fe5b8a21475",
          'tags': ["Software", "AI"],
          'comments': 3,
        ...
        <div>
            {feed.map((feedItem, index) => (
                <div className="d-flex flex-column" key={index}>
                    <Link to={`/explore/posts/${feedItem.postId}`} style={{ textDecoration: 'none' }}>
                        <div className='d-flex'>
                            <div className='avatar-container me-3' style={{ backgroundColor: "grey", width: "5%" }}>
                                <img src={feedItem.avatar} alt="Avatar" /> {/* Use img tag to display the avatar */}
                            </div>
                            <div className='d-flex flex-column'>
                                <h6 className="card-title">{feedItem.name}</h6>
                                <h6 className="card-subtitle mb-2 text-body-secondary">@{feedItem.username} • {feedItem.time}</h6>
                            </div>
                        </div>
                    ...
                    <div className="rounded d-flex mt-3 mb-1 justify-content-center btn-group" role="group" aria-label="Feed Interaction">
                        <button
                            className={`btn btn-light col-3 ${likedPosts[index] ? 'text-dark' : 'text-secondary'}`}
                            style={{ border: "1px solid grey" }}
                            onClick={() => handleLikeClick(index)}
                        >
                            <FontAwesomeIcon icon={faThumbsUp} className={`mr-1 ${likedPosts[index] ? 'text-dark' : 'text-secondary'}`} /> {likesCount[index]}
                        </button>
                        <Link to={`/explore/posts/${feedItem.postId}`} className='btn btn-light col-3' style={{ border: "1px solid grey" }}>
                            <FontAwesomeIcon icon={faCommentAlt} className='text-secondary' /> {feedItem.comments}
                        </Link>
